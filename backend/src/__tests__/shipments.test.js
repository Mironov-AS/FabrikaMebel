...
describe('GET /api/shipments', () => {
  ...
  it('returns existing shipments with items array', async () => {
    await db.query(
      "INSERT INTO shipments (invoice_number, amount, status) VALUES ('INV-LIST', 1000, 'scheduled')"
    );
...
describe('GET /api/shipments/:id', () => {
  it('returns 404 for non-existent shipment', async () => {
    const res = await request(app).get('/api/shipments/999999');
  ...
  it('returns shipment for existing id', async () => {
    const { lastInsertRowid } = await db.runReturning(
      "INSERT INTO shipments (invoice_number, amount, status) VALUES ('INV-GET', 2500, 'scheduled')"
    );
...
describe('POST /api/shipments', () => {
  it('creates a shipment and auto-creates payment, returns 201', async () => {
    ...
    const payment = await db.get('SELECT * FROM payments WHERE shipment_id = $1', [res.body.id]);
  ...
  it('returns 404 when orderId references non-existent order', async () => {
    const res = await request(app)
      .post('/api/shipments')
      .send({ invoiceNumber: 'INV-NO-ORD', orderId: 999999 });
...
describe('PUT /api/shipments/:id/confirm', () => {
  it('confirms a shipment and moves linked order to shipped', async () => {
    ...
    const { lastInsertRowid: shipmentId } = await db.runReturning(
      "INSERT INTO shipments (order_id, order_number, invoice_number, amount, status) VALUES ($1, 'ORD-CONF', 'INV-CONF', 1000, 'scheduled')",
      [orderId]
    );
  ...
  it('returns 404 for non-existent shipment', async () => {
    const res = await request(app).put('/api/shipments/999999/confirm');
    expect(res.status).toBe(404);
  });
});
